...
impl EgraphPass {
    ...
    pub fn run(&mut self) {
        ...
        // Phase 1.5: Iterative range refinement + rewrite cycling
        // Re-walk all values, re-compute ranges with current egraph knowledge,
        // and attempt more rewrites. Repeat until fixed point.
    ...
    ///
    /// Re-walks all values in the egraph, re-computes ranges using transfer
    /// functions, and re-attempts rewrite rules with the refined range info.
    ...
    fn range_refine_iteration(&mut self) -> usize {
        ...
        // Reset range assumptions for a fresh analysis
        self.range_assumptions.clear();

        // Collect all instruction-defined values sorted by InstId for determinism
        ...
        for &(inst_id, value) in &inst_values {
            ...
            // For union nodes, propagate the tightest range from either child
            // through to the union itself
        }

        // Also propagate through union nodes: a union's range is the union
        // of its children's ranges (since either child could be the "real" value)
        ...
        // Phase B: Re-attempt rewrites with refined ranges
        ...
        for value in values_to_rewrite {
            ...
            for rewritten in rewrites {
                if rewritten != value {
                    // Create a new union between the original and the rewritten form
                    ...
                    for inst_id in inst_ids {
                        ...
                        if !output_range.is_unbounded() {
                            if let Some(&result) = self.dfg.inst_results(inst_id).first() {
                                ...
                                let opt_result = value_to_opt_value
                                    .get(&result)
                                    .copied()
                                    .unwrap_or(result);
...
mod tests {
    ...
    fn test_range_propagation_through_constants() {
        ...
        let add_id = dfg.make_inst(add_inst);
        let _sum = dfg.make_inst_result(add_id, Type::I32);
        ...
        // Ranges should have been propagated
        assert!(
            pass.stats.ranges_propagated > 0,
            "Expected ranges to be propagated for constants and their sum"
        );
        println!("\n✓ Range propagation through constants test passed");
    }
    ...
    fn test_range_sensitive_comparison_folding() {
        ...
        let domtree = DominatorTree::from_cfg(block0, &[
